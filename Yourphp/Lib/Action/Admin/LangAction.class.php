...
class LangAction extends AdminbaseAction {
	...
	function insert() {
		...
		if ($id !==false) {
			$db=D('');
			$db =   DB::getInstance();//by yyf 修改默认表前缀
			$db->execute("INSERT INTO `".C('DB_PREFIX')."config`  (`varname`,`info`,`groupid`,`value`,`lang`) VALUES ('site_name','网站名称','2','','".$id."'),
			('site_url','网站网址','2','','".$id."'),
			('logo','网站LOGO','2','./Public/Images/logo.gif','".$id."'),
			('site_email','站点邮箱','2','252208887@qq.com','".$id."'),
			('seo_title','网站标题','2','','".$id."'),
