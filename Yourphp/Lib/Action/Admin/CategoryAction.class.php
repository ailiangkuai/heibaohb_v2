...
class CategoryAction extends AdminbaseAction
{
        ...
        if($this->dao->create())
        {
            ...
            if($id)
            {
				if($_POST['module']=='Page'){
					$_POST['id']=$id;
					$_POST['catid']=$id;//by yyf 同时插入catid防止使用list产生错误
	...
	public function delete() {
		...
		if($this->categorys[$catid]['type']==1){
			...
			$modulename = $module;//by yyf 添加了一个模型名称变量
			$module  = M($module);
			$arrchildid = $this->categorys[$catid]['arrchildid'];
			$where =  "catid in(".$arrchildid.")";
			$count = $module->where($where)->count();
			if($count&&$modulename!="Page") $this->error(L('category_does_not_allow_delete'));
			$this->dao->delete($arrchildid);
			$moduleid = $this->mod[$modulename];//by yyf 在这里使用的之前是"$moduleid = $this->mod[$module];"
			delattach("moduleid =$moduleid and catid in($arrchildid)");//删除媒体文件
