var CONTEXTPATH = '/Public/Image/';//图片路径配置
...
//	alert("\n"+cw+"\n"+ch+"\n"+sw+"\n"+sh)
	...
	arr.push("        <tr drag='false' id='_ButtonRow_"+this.ID+"'><td height='6'>");
	arr.push("            <div id='_DialogButtons_"+this.ID+"' style='text-align:right; padding:4px 2px;'>");
	arr.push("           	<input id='_ButtonOK_"+this.ID+"'  type='hidden' value='确 定'>");
	arr.push("           	<input id='_ButtonCancel_"+this.ID+"'  type='hidden' onclick=\"Dialog.getInstance('"+this.ID+"').close();\" value='取 消'>");
	arr.push("            </div></td></tr>");
	arr.push("      </table></td>");
	...
	if(!this.AlertFlag){
		...
		if(!bgdiv){
			...
			$E.hide(bgdiv);
			pw.$T("body")[0].appendChild(bgdiv);
			bgdiv.style.cssText = "background-color:#333;position:absolute;left:0px;top:0px;opacity:0.4;filter:alpha(opacity=40);width:" + cw + "px;height:" + ch + "px;z-index:991";
		}
	}
	...
	if(!this.AlertFlag){
		...
		if(!flag){
			bgdiv.style.cssText = "background-color:#333;position:absolute;left:0px;top:0px;opacity:0.4;filter:alpha(opacity=40);width:" + cw + "px;height:" + ch + "px;z-index:960";
		}
	}
	//放入队列中，以便于ESC时正确关闭
